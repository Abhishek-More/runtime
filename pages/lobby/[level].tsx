...
export default function Lobby() {
    ...
    if (level) {
      ...
      try {
        ...
        window.location.href =
          "/game/" + data.newLobbyId + "?" + params.toString();
      } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col p-12 justify-center items-center font-monda gap-4">
      ...
      <div className="text-5xl font-bold text-gray-700 font-metal mb-8">
        Enter the Ring: Find a Lobby
      </div>
      <div className="flex flex-col gap-6 bg-white w-full md:max-w-2xl rounded-lg shadow border-gray-600 border-double border-2 px-6 py-8">
        <div className="flex flex-col justify-between items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <div className="font-bold text-gray-700">
              LET&apos;S GET CODING, {nickname}!
            </div>
          </div>
          <div className="font-bold text-gray-400">
            Your Level Selection: {levelQueryMapping[router.query.level]}
          </div>
          {isCreatingLobby ? (
            <Button className="">Creating Your Lobby...</Button>
          ) : (
            <Button className="" onClick={createLobby}>
        ...
        <div>
          {lobbies
            .filter((lobby) => {
              return (
                lobby.lobby_problemset === level &&
                !(
                  lobby.player1_nickname !== "" && lobby.player2_nickname !== ""
                )
              );
            })
            .map((item) => (
